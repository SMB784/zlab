...
if directory_exists(Path(download_dir),save_dir)==True:
    ...
    for root,dirs,files in os.walk(download_dir,topdown=True):
        ...
        for file in sorted(files,key=numerical_sort):
            ...
            if directory_exists(Path(download_dir),dark_dir)==True:
                input_spectrum=pd.DataFrame(spectrum_read.Spectrum(Path(download_dir)/file,dark_frame,calibration).read_spectrum(),dtype=float)
            else:
